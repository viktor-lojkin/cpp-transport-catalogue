...
void JsonReader::FillCatalogue(trans_cat::TransCatalogue& catalogue) {
...
void JsonReader::FillStopDistances(trans_cat::TransCatalogue& catalogue) const {
    const json::Array& arr = GetBaseRequests().AsArray();
    for (auto& request_stops : arr) {
        const auto& request_stops_map = request_stops.AsDict();
        const auto& type = request_stops_map.at("type"s).AsString();
...
std::tuple<std::string_view, std::vector<const trans_cat::Stop*>, bool> JsonReader::FillRoute(const json::Dict& request_map, trans_cat::TransCatalogue& catalogue) const {
...
trans_cat::TransRouter JsonReader::FillRoutingSettings(const json::Node& settings) const {
    trans_cat::TransRouter router_settings{ settings.AsDict().at("bus_wait_time"s).AsInt(), settings.AsDict().at("bus_velocity"s).AsDouble() };

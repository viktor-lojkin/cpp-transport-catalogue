...
std::vector<svg::Polyline> MapRenderer::GetRouteLines(const std::map<std::string_view, const trans_cat::Bus*>& buses, const SphereProjector& sp) const {
    std::vector<svg::Polyline> result;
    size_t color_num = 0;
    for (const auto& [bus_number, bus] : buses) {
        if (bus->stops_of_this_bus.empty()) {
            continue;
        }

        std::vector<const trans_cat::Stop*> route_stops{ bus->stops_of_this_bus.begin(), bus->stops_of_this_bus.end() };
        if (bus->is_circle == false) {
            route_stops.insert(route_stops.end(), std::next(bus->stops_of_this_bus.rbegin()), bus->stops_of_this_bus.rend());
        }
        svg::Polyline line;
...
std::vector<svg::Text> MapRenderer::GetBusLabel(const std::map<std::string_view, const trans_cat::Bus*>& buses, const SphereProjector& sp) const {
    ...
    for (const auto& [bus_number, bus] : buses) {
        ...
        text.SetPosition(sp(bus->stops_of_this_bus[0]->coordinates));
        ...
        underlayer.SetPosition(sp(bus->stops_of_this_bus[0]->coordinates));
        ...
        if (bus->is_circle == false && bus->stops_of_this_bus[0] != bus->stops_of_this_bus[bus->stops_of_this_bus.size() - 1]) {
            svg::Text text2 {text};
            svg::Text underlayer2 {underlayer};
            text2.SetPosition(sp(bus->stops_of_this_bus[bus->stops_of_this_bus.size() - 1]->coordinates));
            underlayer2.SetPosition(sp(bus->stops_of_this_bus[bus->stops_of_this_bus.size() - 1]->coordinates));
...
std::vector<svg::Circle> MapRenderer::GetStopsSymbols(const std::map<std::string_view, const trans_cat::Stop*>& stops, const SphereProjector& sp) const {
...
std::vector<svg::Text> MapRenderer::GetStopsLabels(const std::map<std::string_view, const trans_cat::Stop*>& stops, const SphereProjector& sp) const {
    std::vector<svg::Text> result;
    svg::Text text;
    svg::Text underlayer;
    for (const auto& [stop_name, stop] : stops) {
...
svg::Document MapRenderer::GetSVG(const std::map<std::string_view, const trans_cat::Bus*>& buses) const {
    svg::Document result;
    std::vector<geo::Coordinates> route_stops_coord;
    std::map<std::string_view, const trans_cat::Stop*> all_stops;

    for (const auto& [bus_number, bus] : buses) {
        if (bus->stops_of_this_bus.empty()) {
            continue;
        }
        for (const auto& stop : bus->stops_of_this_bus) {
